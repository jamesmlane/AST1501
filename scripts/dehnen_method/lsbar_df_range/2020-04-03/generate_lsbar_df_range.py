...
with a range of radial force fractions, but the same canonical pattern speed 
and 
'''
...
for i in range( len(_LSBAR_AF_RANGE) ):
    
    # Make the log file
    output_str = '_AF_'+_LSBAR_AF_RANGE_STR[i]
    _LOGFILE = open('./log'+output_str+'.txt','w')

    # Make the potential
    _LSBAR_POT = potential.DehnenBarPotential(omegab=_LSBAR_OMEGAB, 
        rb=_LSBAR_RB, Af=_LSBAR_AF_RANGE[i], 
    ...
    write_dict = {'NCORES':_NCORES,
                  ...
                  'BAR_OMEGAB':_LSBAR_OMEGAB,
                  'BAR_AF':_LSBAR_AF_RANGE_STR[i]
                  }
