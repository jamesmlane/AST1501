...
winding spiral arms. Evaluate just a small range radial range, with a range of
amplitudes. Used to test whether the resulting velocity fields scale with 
amplitude.
'''
...
_POT_AMP = np.array([0.5,1.0,1.5,2.0]) 0.136* apu.M_sun / (apu.pc**3)
...
_POT_LIFETIME = np.array([0.120,0.240]) * apu.Gyr # / gpconv.time_in_Gyr(ro=8,vo=220)
_POT_LIFETIME_PRINT = np.array([0.120,0.240])
_POT_SIGMA = _POT_LIFETIME / 5.6
_POT_T0 = np.array([-0.86,-0.29])*apu.Gyr #  / gpconv.time_in_Gyr(ro=8,vo=220) # 
_POT_T0_PRINT = np.array([-0.86,-0.29])
_POT_BETA = -0.1
    ...
    for j in range( len(_POT_LIFETIME) ):
        for k in range( len(_POT_AMP) ):
            ...
            write_params = [_NCORES,_TIMES,_N_ARMS[i],_POT_PHI0[i],_POT_LIFETIME,
                    _POT_SIGMA[j],_POT_T0[j],_POT_AMP[k],_POT_H,_POT_RS,_POT_ALPHA,
